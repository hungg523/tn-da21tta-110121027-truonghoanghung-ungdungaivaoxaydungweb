...
{
    public class LoginHandler : IRequestHandler<LoginRequest, Result<LoginDTO>>
    {
        private readonly IAuthRepository authRepository;
        private readonly IJwtService jwtService;
        private readonly IUserRepository userRepository;

        public LoginHandler(IAuthRepository authRepository, IJwtService jwtService, IUserRepository userRepository)
        {
        ...
        public async Task<Result<LoginDTO>> Handle(LoginRequest request, CancellationToken cancellationToken)
        {
            ...
            if (user is null || user.IsActived == 0) AppleException.ThrowNotFound();
